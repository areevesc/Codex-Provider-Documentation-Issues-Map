...
export function ProviderIssueForm({ issue }: ProviderIssueFormProps) {
  ...
  const deleteProviderIssue = useAppStore((s) => s.deleteProviderIssue);
  ...
  function handleDeleteIssue() {
    const confirmed = window.confirm('Delete this provider issue? This cannot be undone.');
    if (!confirmed) return;
    deleteProviderIssue(issue.id);
    navigate(`/providers/${issue.providerId}`);
  }
    ...
    <>
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            {label?.description && <p className="text-xs text-ink-muted">{label.description}</p>}
          </div>
      ...
      >
        ...
        <div className="fixed inset-0 flex items-center justify-center p-3 sm:p-6">
          <DialogPanel className="flex max-h-full w-full max-w-6xl flex-col overflow-hidden rounded-lg border border-line bg-surface-raised shadow-2xl">
            <div className="flex items-center justify-between gap-3 border-b border-line px-4 py-3">
              <DialogTitle className="min-w-0 truncate text-sm font-semibold text-ink">
