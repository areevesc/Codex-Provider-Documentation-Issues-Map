...
export function ProviderHistorySection({ providerId }: ProviderHistorySectionProps) {
  const issues = useAppStore(
    useShallow((s) => getHistoricalProviderIssuesForProvider(s, providerId)),
  );
  const [open, setOpen] = useState(true);

  const { resolved, sorted } = useMemo(() => {
    ...
    <section className="mt-8">
      <header>
        ...
        >
          ...
          <div className="shrink-0 text-xs text-ink-muted">{resolved} resolved</div>
        </button>
      </header>
